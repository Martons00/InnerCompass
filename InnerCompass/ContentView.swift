        ...
        VStack {
            Button(action: {
                self.viewController?.present(style: .fullScreen, transitionStyle: .crossDissolve) {
                    BallonView(index: Int(marker.degrees))
                }
            }) {
                Image(marker.label)
                    .resizable()
                    .frame(width: 50 * CGFloat(fontSizerForDevice()),height: 50 * CGFloat(fontSizerForDevice()))
                    .rotationEffect(self.textAngle())
            }
            
            .padding(.bottom, 220 * CGFloat(fontSizerForDevice()))
